...
@interface IRImagePickerController (Private)
...
- (NSArray *)ir_indexPathsFromIndexesWithSection:(NSUInteger)section
{
...
- (NSArray *)ir_indexPathsForElementsInRect:(CGRect)rect
{
...
@interface IRAssetsViewController () <PHPhotoLibraryChangeObserver, UICollectionViewDelegateFlowLayout>
...
@implementation IRAssetsViewController
...
{
    if ([self.imagePickerController.delegate respondsToSelector:@selector(ir_imagePickerController:didFinishPickingAssets:)]) {
        [self.imagePickerController.delegate ir_imagePickerController:self.imagePickerController
...
- (void)updateSelectionInfo
{
    ...
    if (selectedAssets.count > 0) {
        ...
        if (selectedAssets.count > 1) {
            format = NSLocalizedStringFromTableInBundle(@"assets.toolbar.items-selected", @"IRImagePickerController", bundle, nil);
        } else {
            format = NSLocalizedStringFromTableInBundle(@"assets.toolbar.item-selected", @"IRImagePickerController", bundle, nil);
        }
...
- (void)updateFetchRequest
{
    if (self.assetCollection) {
        ...
        switch (self.imagePickerController.mediaType) {
            case IRImagePickerMediaTypeImage:
                options.predicate = [NSPredicate predicateWithFormat:@"mediaType == %ld", PHAssetMediaTypeImage];
                break;
                
            case IRImagePickerMediaTypeVideo:
...
- (void)updateCachedAssets
{
    ...
    if (delta > CGRectGetHeight(self.collectionView.bounds) / 3.0) {
        ...
        [self computeDifferenceBetweenRect:self.previousPreheatRect andRect:preheatRect addedHandler:^(CGRect addedRect) {
            NSArray *indexPaths = [self.collectionView ir_indexPathsForElementsInRect:addedRect];
            [addedIndexPaths addObjectsFromArray:indexPaths];
        } removedHandler:^(CGRect removedRect) {
            NSArray *indexPaths = [self.collectionView ir_indexPathsForElementsInRect:removedRect];
            ...
            if (![collectionChanges hasIncrementalChanges] || [collectionChanges hasMoves]) {
                ...
                [self.collectionView performBatchUpdates:^{
                    ...
                    if ([removedIndexes count]) {
                        [self.collectionView deleteItemsAtIndexPaths:[removedIndexes ir_indexPathsFromIndexesWithSection:0]];
                    }
                    
                    NSIndexSet *insertedIndexes = [collectionChanges insertedIndexes];
                    if ([insertedIndexes count]) {
                        [self.collectionView insertItemsAtIndexPaths:[insertedIndexes ir_indexPathsFromIndexesWithSection:0]];
                    }
                    
                    NSIndexSet *changedIndexes = [collectionChanges changedIndexes];
                    if ([changedIndexes count]) {
                        [self.collectionView reloadItemsAtIndexPaths:[changedIndexes ir_indexPathsFromIndexesWithSection:0]];
                    }
                } completion:NULL];
            }
...
{
    IRAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"AssetCell" forIndexPath:indexPath];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    if (kind == UICollectionElementKindSectionFooter) {
        ...
        switch (self.imagePickerController.mediaType) {
            case IRImagePickerMediaTypeAny:
            {
                NSString *format;
                if (numberOfPhotos == 1) {
                    if (numberOfVideos == 1) {
                        format = NSLocalizedStringFromTableInBundle(@"assets.footer.photo-and-video", @"IRImagePickerController", bundle, nil);
                    } else {
                        format = NSLocalizedStringFromTableInBundle(@"assets.footer.photo-and-videos", @"IRImagePickerController", bundle, nil);
                    }
                } else if (numberOfVideos == 1) {
                    format = NSLocalizedStringFromTableInBundle(@"assets.footer.photos-and-video", @"IRImagePickerController", bundle, nil);
                } else {
                    format = NSLocalizedStringFromTableInBundle(@"assets.footer.photos-and-videos", @"IRImagePickerController", bundle, nil);
                }
            ...
            case IRImagePickerMediaTypeImage:
            {
                ...
                NSString *format = NSLocalizedStringFromTableInBundle(key, @"IRImagePickerController", bundle, nil);
...
{
    if ([self.imagePickerController.delegate respondsToSelector:@selector(ir_imagePickerController:shouldSelectAsset:)]) {
        PHAsset *asset = self.fetchResult[indexPath.item];
        return [self.imagePickerController.delegate ir_imagePickerController:self.imagePickerController shouldSelectAsset:asset];
    }
...
{
    IRImagePickerController *imagePickerController = self.imagePickerController;
    ...
    if (imagePickerController.allowsMultipleSelection) {
        ...
        if ([imagePickerController.delegate respondsToSelector:@selector(ir_imagePickerController:didFinishPickingAssets:)]) {
            [imagePickerController.delegate ir_imagePickerController:imagePickerController didFinishPickingAssets:@[asset]];
        }
    }
    
    if ([imagePickerController.delegate respondsToSelector:@selector(ir_imagePickerController:didSelectAsset:)]) {
        [imagePickerController.delegate ir_imagePickerController:imagePickerController didSelectAsset:asset];
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didDeselectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([imagePickerController.delegate respondsToSelector:@selector(ir_imagePickerController:didDeselectAsset:)]) {
        [imagePickerController.delegate ir_imagePickerController:imagePickerController didDeselectAsset:asset];
    }
}
